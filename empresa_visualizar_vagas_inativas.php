...
$result = mysqli_query($mysqli, "SELECT VAGA_CRIADA.RESUMO,VAGA_CRIADA.ID_VAGAC,PROFISSAO.AREA,PROFISSAO.NOME,PERGUNTAS_EMPRESA.PERGUNTA,VAGA_CRIADA.QTDA_OFERTADA,VAGA_CRIADA.STATUS FROM VAGA_CRIADA
INNER JOIN PERGUNTAS_EMPRESA ON VAGA_CRIADA.ID_PERGUNTA1 = PERGUNTAS_EMPRESA.ID_PERGUNTA
INNER JOIN PROFISSAO ON PERGUNTAS_EMPRESA.ID_PROFISSAO = PROFISSAO.ID_PROFISSAO
WHERE PERGUNTAS_EMPRESA.ID_EMPRESA = '$id_empresa' AND PERGUNTAS_EMPRESA.ID_PERGUNTA = VAGA_CRIADA.ID_PERGUNTA1 AND VAGA_CRIADA.STATUS LIKE 'INATIVA'");

//$result = mysqli_query($mysqli, "SELECT VAGA_CRIADA.ID_VAGAC,PROFISSAO.AREA,PROFISSAO.NOME,PERGUNTAS_EMPRESA.PERGUNTA,VAGA_CRIADA.QTDA_OFERTADA,VAGA_CRIADA.STATUS FROM VAGA_CRIADAPERGUNTAS_EMPRESA,PROFISSAO WHERE PERGUNTAS_EMPRESA.ID_EMPRESA = '$id_empresa' AND PERGUNTAS_EMPRESA.ID_PROFISSAO = PROFISSAO.ID_PROFISSAO AND PERGUNTAS_EMPRESA.STATUS LIKE 'ATIVA'");
								...
								<?php
									while($res = mysqli_fetch_array($result)) {
										echo "<tr>";
											...
											echo "<td><a class=\"btn btn-xs btn-danger\" role=\"button\">".$res['STATUS']."</a>";
											echo "<td><a href=\"empresa_editar_vaga.php?ID_VAGAC=$res[ID_VAGAC]\" class=\"btn btn-xs btn-warning\" role=\"button\">Editar</a> |
											 <a href=\"_php/manter-vaga/ativar_vaga.php?ID_VAGAC=$res[ID_VAGAC]\"class=\"btn btn-xs btn-success\" role=\"button\" onClick=\"return confirm('Você tem certeza que deseja ativar essa vaga?')\">	Ativar</a></td>";
										echo "</tr>";
									}
								 ?>
								 </tbody>
